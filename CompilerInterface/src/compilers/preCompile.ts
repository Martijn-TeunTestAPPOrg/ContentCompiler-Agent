...
export const preCompile = async (app: Probot, context: Context<'pull_request'>, git: SimpleGit) => {
    ...
    // Step 12: Copy the changed files to the storage folder
	...
	// Step 13: Remove the cloned repository
	try {
	...
	// Step 14: Move the temp storage folder to the cloned repo folder
	...
	// Step 14.1: Create the content source folder if it doesn't exist
	...
	// Step 15: Compile the content
	await compileContent(logger, config.preCompileCommand);

	// Step 16: Create a review with the compiled content
	...
	// Step 17: Remove the temp storage folder
	await clearTempStorage(app, logger, config.contentRootDir, config.tempStorageDir, config.datasetDir, config.compilerDir);
