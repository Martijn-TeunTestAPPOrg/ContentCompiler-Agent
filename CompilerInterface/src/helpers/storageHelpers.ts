...
export function clearTempStorage(app: Probot, logger: winston.Logger, contentRootDir: string, tempStorageDir: string, datasetDir: string, compilerDir: string) {
    try {
        ...
        // Remove the compiler folder if it exists
        if (fs.existsSync(compilerDir)) {
            deleteFolderRecursive(app, logger, compilerDir);
        }
