...
import { getPayloadInfo, clearTempStorage, cloneRepo, checkoutBranch, compileContent, deleteFolderRecursive, copySpecificFiles, getChangedFiles, checkIllegalChanges, postPRReview, hideBotComments } from "./helpers.js";
...
export const preCompile = async (app: Probot, context: Context<'pull_request'>) => {
    ...
    // Step 4: Checkout to the 'head' branch
    await checkoutBranch(app, headBranch, contentRootDir);

    // Step 5: Get the changed files in the PR
    changedFiles = await getChangedFiles(app, context, baseBranch, headBranch);
    ...
    // Step 6.1: Post a review if there are illegal changes
    ...
    // Step 7: Copy the changed files to the storage folder
    ...
    // Step 8: Remove the cloned repository
    try {
    ...
    // Step 13: Remove the temp storage folder
    await clearTempStorage(app, contentRootDir, tempStorageDir, datasetDir);
